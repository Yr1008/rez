...
(function(){
  ...
  function normalizePadding(){
    ...
    document.querySelectorAll('section[data-screen-label]').forEach(function(s){
      ...
      if (pt > 8 || pb > 8) {
        s.style.setProperty('padding-top', pad + 'px', 'important');
  ...
  function buildMenu(){
    ...
    anchors.forEach(function(a){
      if (a.closest && a.closest('.navdrop')) return; /* dropdown items live under the Industries group */
      var label = a.textContent.trim();
      /* Fold the industries into the menu under one heading so mobile shows
         ONE nav, not a separate floating industry bar. */
      if (/^industries$/i.test(label)){
        var acc = document.createElement('button');
        acc.type = 'button'; acc.className = 'rzmenu-acc'; acc.setAttribute('data-open','0');
        acc.innerHTML = 'Industries <svg class="rzmenu-chev" viewBox="0 0 12 8" fill="none" aria-hidden="true"><path d="M1 1l5 5 5-5" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>';
        ...
        INDUSTRIES.forEach(function(pair){
