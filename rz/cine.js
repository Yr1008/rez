...
*/
(function(){
  /* In the editor preview we still run the full choreography (cards, word cycle, outro);
     only the scroll-restore hijack below is skipped so it can't fight the editor. */
  var PREVIEW = location.pathname.indexOf('/v1/design/') !== -1;
  ...
  function render(p){
    ...
    /* The label block is bottom-anchored and, on desktop, site.css sets its h2
       to a fixed 72px - wide headlines ("When customers can bank on") then span
       further than the lower cards' fixed data-x offsets, so the cards settle
       on top of the text. Clearance is therefore enforced against the measured
       headline every frame: a card sharing the label's vertical band is pushed
       out to at least 24px clear of the text; when the viewport has no room
       beside the text, it clears vertically instead (below the label, or above
       it when the bottom would clip). The correction is scaled by the label's
       opacity so cards glide back as the label fades instead of snapping. */
    var labelRect=null, labelOp=0, lwEl=document.getElementById('cinelabelwrap');
    if(lwEl){ labelOp=parseFloat(lwEl.style.opacity||'0')||0;
      if(labelOp>0.05){ var lh2=lwEl.querySelector('h2'); if(lh2) labelRect=lh2.getBoundingClientRect(); } }
    for(var i=0;i<cards.length;i++){ var c=cards[i];
      ...
      var cy=lp(y+10,y,rin)+(p-st)*-4, tx=lp(x*0.5,x,rin);
      if(labelRect && x!==0 && rin>0){
        var vwPx=innerWidth/100, vhPx=innerHeight/100;
        var cw=c.offsetWidth||290, chh=c.offsetHeight||200;
        var cTop=innerHeight/2+cy*vhPx-chh/2, cBot=cTop+chh;
        if(cBot>labelRect.top-10 && cTop<labelRect.bottom+10){
          /* The floor is scaled by g, which holds at 1 while the label is
             legible and releases only as it fades - so entering cards surface
             at the clearance line instead of inside the text, and settle back
             to their natural offsets once the label is gone. */
          var g=ez(cl(labelOp/0.2));
          var needPx=labelRect.width/2+cw/2+20, maxPx=innerWidth/2-cw/2-8;
          if(needPx<=maxPx){
            var floorVw=(needPx*g)/vwPx;
            if(Math.abs(tx)<floorVw) tx=(x<0?-1:1)*floorVw;
          } else {
            var floorVw2=(maxPx*g)/vwPx;
            if(Math.abs(tx)<floorVw2) tx=(x<0?-1:1)*floorVw2;
            var down=labelRect.bottom+12-cTop, up=cBot-(labelRect.top-12);
            var dy=(cBot+down<=innerHeight-8)?down:-up;
            cy+=(dy*g)/vhPx;
          }
        }
      }
      var sc=lp(.88,1,rin)*lp(1,.96,ez((p-0.84)/0.12));
  ...
  if(!PREVIEW){ try{ if('scrollRestoration' in history) history.scrollRestoration='manual'; }catch(_){} }
  ...
  if(!PREVIEW){ try{ window.scrollTo(0,0); }catch(_){} }
  ...
  if(!PREVIEW){ try{ document.documentElement.classList.add('cine-lock'); }catch(_){} }
